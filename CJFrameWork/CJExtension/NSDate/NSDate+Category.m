...
+ (NSString *) timeStrStyle1:(long long)time
{
...
+ (NSString *) timeStrStyle2:(long long)time
{
    NSDate * date=[NSDate dateWithTimeIntervalSince1970:time];
    NSCalendar * calendar=[[NSCalendar alloc]initWithCalendarIdentifier:NSGregorianCalendar];
    NSInteger unitFlags = NSMonthCalendarUnit | NSDayCalendarUnit|NSYearCalendarUnit|NSHourCalendarUnit|NSMinuteCalendarUnit|NSSecondCalendarUnit|NSWeekCalendarUnit|NSWeekdayCalendarUnit;
    ...
    long week=[component week];
    long weekday=[component weekday];
    
    NSDate * today=[NSDate date];
    component=[calendar components:unitFlags fromDate:today];
    long t_year=[component year];
    long t_day=[component day];
    long t_week=[component week];
    ...
    if(distance<60){
        ...
        return [NSString stringWithFormat:@"%ld分钟前",distance/60];
    }else if(distance < 24 * 60 * 60 && ((t_day - day) == 0)){
        return [NSString stringWithFormat:@"今天%ld:%02ld",hour,minute];
    }else if(distance < 48 * 60 * 60 && ((t_day - day) == 1)){
        return [NSString stringWithFormat:@"昨天%ld:%02ld",hour,minute];
    }else if(distance < 72* 60 *60 && ((t_day - day) == 2)){
        return [NSString stringWithFormat:@"前天%ld:%02ld",hour,minute];
        ...
        return [NSString stringWithFormat:@"星期%@%ld:%02ld",daystr,hour,minute];
    }else if(year==t_year){
